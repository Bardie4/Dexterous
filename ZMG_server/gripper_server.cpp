...
class finger{
  ...
  static void zmq_update(gpointer input_ptr){
    ...
    auto orc_deserial = Getsimple_instructions(buf);
    double out_data1 = orc_deserial->data1();

    double out_data2 = orc_deserial->data2();
    std::cout << (double)out_data1 <<"    "<<out_data2 << std::endl;
    snprintf ((char *) message, 1000, "%d %d %d %d %d %d", zmq->identity, zmq->controller_select, zmq->data1, zmq->data2, zmq->data3, zmq->data4);
    s_sendmore (zmq->publisher, "B");
    s_send (zmq->publisher, message);
