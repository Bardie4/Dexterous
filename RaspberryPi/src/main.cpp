  ...
  public:
    ...
    Finger(int identity)
      ...
      //*********FIND START POINT OF SENSOR 1**********
			...
			torque_cmd[2]=(uint8_t) 130;  //link2
			pthread_mutex_lock(&periphLock);
  ...
  public:
    ...
    void* run(){
        ...
        handStates.clear();
            ...
            if ( angle1Temp < (*zeroAngle[i][0] - 3.14159/4.0 )){              //Check if it has crossed zero point
              zeroCross[i][0] = 1;
            }else{
              zeroCross[i][0] = 0;
            }
            ...
            if (angle2Temp <  (*zeroAngle[i][1] - 0.3.14159/4.0)){              //Check if it has crossed zero point
              zeroCross[i][1] = 1;
            }else{
              zeroCross[i][1] = 0;
            }
